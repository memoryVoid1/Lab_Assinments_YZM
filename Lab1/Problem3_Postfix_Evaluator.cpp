...
int evaluatePostfix(const string& expression) {
    ...
    stack<int> postfixExpression;
    stringstream ss(expression);
    string token;
    while (ss >> token) {
        if (token == "+" || token == "-" || token == "*" || token == "/") {
            int num2 = postfixExpression.top();
            postfixExpression.pop();
            int num1 = postfixExpression.top();
            postfixExpression.pop();
            if (token == "+") {
                postfixExpression.push(num1 + num2);
            }
            else if (token == "-") {
                postfixExpression.push(num1 - num2);
            }
    ...
    int finNum = postfixExpression.top();
    postfixExpression.pop();
